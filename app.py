...
def auth_callback():
    """
    Rota de Callback - O Google redireciona o usuário para cá após o login.
    Aqui validamos o usuário e criamos a sessão.
    (Esta URL deve ser a mesma que você configurou na Etapa 1.4)
    """
    try:
        # 1. Pega o token de acesso que o Google nos enviou
        token = oauth.google.authorize_access_token()
        
        # 2. Pega o userinfo de dentro do objeto token
        user_info = token.get('userinfo')

        # 2.1 Verifica se o user_info realmente veio
        if not user_info:
            logging.error("Falha ao obter 'userinfo' do token do Google. O token não o continha.")
            flash('Ocorreu um erro ao ler os dados do Google. Tente novamente.')
            return redirect(url_for('login'))

        # 3. --- PONTO CRÍTICO: Validação do Domínio (COM MÚLTIPLOS DOMÍNIOS) ---
        user_email = user_info.get('email', '')
        
        # (MUDANÇA 1)
        # Define uma LISTA de domínios permitidos
        DOMINIOS_PERMITIDOS = [
            '@soucarbonell.com.br',
        ...
        # (MUDANÇA 2)
        # Verifica se o e-mail do usuário termina com QUALQUER um dos domínios da lista
        email_valido = False
        if user_email:
            email_lower = user_email.lower()
            # Loop que verifica se o email termina com algum dos domínios permitidos
            for domain in DOMINIOS_PERMITIDOS:
                if email_lower.endswith(domain):
                    email_valido = True
                    break # Encontrou um domínio válido, pode parar de verificar

        if email_valido:
            # --- Login bem-sucedido (Estudante) ---
            
            # 4. Cria a sessão do usuário
            session['usuario_logado'] = True
            session['aluno_id'] = user_email # Usamos o email como ID para estudantes
            session['aluno_nome'] = user_info.get('name', 'Estudante')
            ...
            flash("Acesso negado. Apenas contas institucionais (@soucarbonell ou @colegiocarbonell) são permitidas.")
            return redirect(url_for('login'))
